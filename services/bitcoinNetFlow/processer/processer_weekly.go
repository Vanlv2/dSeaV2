...
// Hàm xử lý phân tích dữ liệu và gửi lên blockchain theo tuần
func processAndSendWeeklyDataToBlockchain(timeInterval int) {
	...
	if realTimeRecordCount > 0 || historicalRecordCount > 0 {
		...
		go func() {
			sendData.SendDataToSMC(realTimeResult, historicalResult, "weekly")
		}()
	}
}
