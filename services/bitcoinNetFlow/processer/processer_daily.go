...
// Hàm xử lý phân tích dữ liệu và gửi lên blockchain
func processAndSendDailyDataToBlockchain(timeInterval int) {
	...
	if realTimeRecordCount > 0 || historicalRecordCount > 0 {
		...
		go func() {
			sendData.SendDataToSMC(realTimeResult, historicalResult, "daily")
		}()
	}
}
